...
from app.forms import LoginForm, RegisterForm, ExtendedLoginForm
from app.models import User,Role,AvaliableVacationDay, AvaliableVacationDaySchema, State, UserAvaliableVacationDays, UserAvaliableVacationDaysSchema
from flask_security import Security, SQLAlchemyUserDatastore, UserMixin, RoleMixin, login_required, utils, roles_accepted, login_user, logout_user, current_user
...
def before_first_request():	
    ...
    if not user_datastore.get_user('admin@admin.com'):
        user_datastore.create_user(email='admin@admin.com', password=utils.encrypt_password('admin'))
...
def set_date_vacation_ajax():
	...
	if (str(operation) == "insert"):
		...
		user_vacation = UserAvaliableVacationDays(vacation_day_id=date,user_id=current_user.id,state_id=state)
	...
	if (str(operation) == "delete"):
		...
		this_date = db.session.query(UserAvaliableVacationDays).filter_by(
			vacation_day_id=date.id,
...
def get_date_vacation_ajax():
	...
	for d in vacation_dates:	
		row = {}
		vacationID = d.vacation_day_id
		date = db.session.query(AvaliableVacationDay.avaliable_day).filter_by(id=vacationID).scalar()
